...
export default function Tasks() {
    ...
    const handleUploadFallback = (file, type, toastId) => {
        ...
        // 1. Try Base64 for SMALL files (Persistent in Firestore)
        // Limit to 250KB to respect Firestore 1MB limit per doc
        if (file.size < 250 * 1024) {
            const reader = new FileReader();
            reader.onloadend = () => {
                const base64String = reader.result;
                const newAttachment = {
                    name: file.name, // No need to label (Local) if it's functional
                ...
                toast.success("Disimpan di Database (Backup)", { id: toastId, icon: '⚠️' });
            };
            reader.readAsDataURL(file);
            return;
        }

        // 2. Use Object URL for larger files (Temporary Preview)
        const objectUrl = URL.createObjectURL(file);
        const newAttachment = {
            name: file.name + ' (Preview All - Gagal Upload)',
            url: objectUrl,
            type: type,
            size: file.size,
            uploadedAt: new Date().toISOString()
        ...
        toast.success("Mode Preview (Gagal Upload)", { id: toastId, icon: '⚠️' });
    };
    ...
    try {
        ...
        uploadTask.on('state_changed',
            ...
            async () => {
                try {
                    ...
                    toast.success("File upload selesai!", { id: toastId });
                } catch (urlError) {
        ...
        toast.error(`Error initialization: ${error.message}`, { id: toastId });
    }
};
                    ...
                    {!loading && totalPages > 1 && (
                        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
                            <div className="text-sm text-slate-500">
                                Showing <span className="font-bold text-slate-800">{startIndex + 1}</span> - <span className="font-bold text-slate-800">{Math.min(endIndex, allFilteredTasks.length)}</span> dari <span className="font-bold text-slate-800">{allFilteredTasks.length}</span> tugas
                            </div>
